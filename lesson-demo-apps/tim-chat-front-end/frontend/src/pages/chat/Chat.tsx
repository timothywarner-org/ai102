...
const Chat = () => {
  ...
  const [isInteractionsEnabled, setIsInteractionsEnabled] = useState<boolean>(true)
  const processMessagesRef = useRef<ChatMessage[]>([])
  const [selectedAnswer, setSelectedAnswer] = useState<string>()
  const [processMessages, setProcessMessages] = useState<messageStatus>(messageStatus.NotRunning)
  const [showLoadingBubble, setShowLoadingBubble] = useState<boolean>(false)
  ...
  const [showApiVisualizer, setShowApiVisualizer] = useState<boolean>(false)
  const [apiVisualizerData, setApiVisualizerData] = useState<{
    request: { messages: ChatMessage[]; timestamp: number };
    response?: {
      answer: string;
      latency: number;
      tokens?: {
  ...
  const learningPanelStore = useLearningPanelStore();

  const errorDialogContentProps = {
    type: DialogType.close,
    title: error?.title,
    closeButtonAriaLabel: 'Close',
    subText: error?.subtitle
  }
  ...
  const clearLearningPanel = () => {
    learningPanelStore.clearContent();
  };
  ...
  const processResultMessage = async (resultMessage: ChatMessage, userMessage: ChatMessage, conversationId?: string) => {
    ...
    // Clear learning panel if there's an error
    if (resultMessage.role === ERROR) {
      clearLearningPanel();
      return;
    }
    ...
    if (resultMessage.role === ASSISTANT) {
      ...
      // Update learning panel with meaningful content derived from the conversation
      if (typeof userMessage.content === "string" && typeof assistantContent === "string") {
        // Set the topic from user's question
        learningPanelStore.setTopic(userMessage.content);
        
        // Generate enrichments based on the actual response
        const enrichments = [];

        // Add token usage analytics if available
        if (resultMessage.token_usage) {
          const tokenCost = calculateTokenCost(resultMessage.token_usage);
          enrichments.push({
            type: 'learning_path' as const,
            title: '📊 Token Usage',
            content: `Prompt: ${resultMessage.token_usage.prompt_tokens} tokens
Completion: ${resultMessage.token_usage.completion_tokens} tokens
Total: ${resultMessage.token_usage.total_tokens} tokens
Cost: ${tokenCost}`,
            relatedLinks: [
              'https://learn.microsoft.com/azure/ai-services/openai/concepts/tokens'
            ]
          });
        }

        // Add investigation tips based on detected topics
        const topics = detectTopics(assistantContent);
        if (topics.length > 0) {
          enrichments.push({
            type: 'investigation_tip' as const,
            title: '🔍 Related Topics',
            content: topics.map(t => `- ${t}`).join('\n'),
            relatedLinks: topics.map(t => `https://learn.microsoft.com/azure/ai-services/openai/concepts/${t.toLowerCase().replace(/\s+/g, '-')}`)
          });
        }
        
        // Update enrichments in store
        learningPanelStore.setEnrichments(enrichments);

        // Update resources based on detected topics and code presence
        const resources = [
          {
            title: 'Azure OpenAI Documentation',
            description: 'Official documentation covering the concepts discussed in this response',
            url: 'https://learn.microsoft.com/azure/cognitive-services/openai/',
            type: 'documentation' as const
          }
        ];

        // Add SDK resources if code is present
        if (assistantContent.includes('```')) {
          resources.push({
            title: 'Azure OpenAI SDK for JavaScript',
            description: 'Official SDK with examples and implementations',
          ...
          resources.push({
            ...
            description: 'Collection of examples and best practices',
        ...
        learningPanelStore.setResources(resources);

        // Extract and enhance code examples from the response
        const codeBlocks = assistantContent.match(/```[\s\S]*?```/g) || [];
        const examples = codeBlocks.map((block, index) => {
          const language = block.split('\n')[0].replace('```', '').trim() || 'typescript';
          const code = block.replace(/```[\s\S]*?\n/, '').replace(/```$/, '').trim();
          
          // Try to extract a meaningful title from the code or context
          const titleMatch = code.match(/\/\/\s*(.+)/) || code.match(/\/\*\s*(.+?)\s*\*\//) || [`Example ${index + 1}`];
          const title = titleMatch[1] || `Example ${index + 1}`;
          
          // Generate a meaningful description
          const description = generateCodeDescription(code, language);
        ...
        // Update examples in store
        learningPanelStore.setExamples(examples);
      }

      if (resultMessage.token_usage) {
        setApiVisualizerData(prev => ({
          ...
          response: {
            ...
            latency: Date.now() - (prev?.request.timestamp || Date.now()),
            tokens: {
              prompt: resultMessage.token_usage.prompt_tokens,
              completion: resultMessage.token_usage.completion_tokens,
              total: resultMessage.token_usage.total_tokens
            }
          }
        }));
  ...
  const makeApiRequestWithoutCosmosDB = async (question: string, id?: string) => {
    clearLearningPanel();
    learningPanelStore.setIsFirstLoad(false);
    setIsLoading(true)
              ...
              if (obj !== '' && obj !== '{}') {
                ...
                if (result.choices?.length > 0) {
                  ...
                  result.choices[0].messages.forEach(resultObj => {
                    processResultMessage(resultObj, userMessage, id)
                  })
  ...
  const makeApiRequestWithCosmosDB = async (question: string, id?: string) => {
    clearLearningPanel();
    learningPanelStore.setIsFirstLoad(false);
    setIsLoading(true)
    setShowLoadingMessage(true)
    ...
    let conversation
    if (id) {
      conversation = appStateContext?.state?.chatHistory?.find(conv => conv.id === id)
      if (!conversation) {
        console.error('Conversation not found.')
        setIsLoading(false)
        setShowLoadingMessage(false)
    ...
    try {
      const response = id
        ? await historyGenerate(request, abortController.signal, id)
      ...
      if (!response?.ok) {
        ...
        if (id) {
          resultConversation = appStateContext?.state?.chatHistory?.find(conv => conv.id === id)
  ...
  const setErrorMsg = (msg: { title: string, subtitle: string } | null) => {
    if (msg) {
      setError(msg)
    } else {
      setError(undefined)
  ...
  // Helper function to detect topics in the content
  const detectTopics = (content: string): string[] => {
    const topics = new Set<string>();
    
    // Common Azure OpenAI topics to detect
    const topicPatterns = [
      { pattern: /\b(prompt|prompting|prompt design)\b/i, topic: 'Prompt Engineering' },
      { pattern: /\b(token|tokens|tokenization)\b/i, topic: 'Token Usage' },
      { pattern: /\b(model|models|gpt)\b/i, topic: 'Models' },
      { pattern: /\b(fine(-|\s)?tun(e|ing))\b/i, topic: 'Fine-tuning' },
      { pattern: /\b(embedding|embeddings)\b/i, topic: 'Embeddings' },
      { pattern: /\b(api|apis|endpoint|endpoints)\b/i, topic: 'API Reference' },
      { pattern: /\b(security|authentication|auth)\b/i, topic: 'Security' },
      { pattern: /\b(cost|pricing|quota|quotas)\b/i, topic: 'Pricing and Quotas' }
    ];
  ...
  // Helper function to generate code descriptions
  const generateCodeDescription = (code: string, language: string): string => {
    ...
    if (functions.length > 0 || classes.length > 0) {
      ...
      if (functions.length > 0) {
        components.push(`functions: ${functions.join(', ')}`);
      }
      if (classes.length > 0) {
        components.push(`classes: ${classes.join(', ')}`);
      }
      return `${language} implementation showing ${components.join(' and ')}`;
    }
    
    return `Example ${language} code snippet`;
  };

  // Helper function to calculate estimated token cost
  const calculateTokenCost = (tokenUsage: { prompt_tokens: number; completion_tokens: number; total_tokens: number }) => {
    // GPT-4 pricing (as of 2024)
    const PROMPT_TOKEN_COST = 0.00003; // $0.03 per 1K tokens
    const COMPLETION_TOKEN_COST = 0.00006; // $0.06 per 1K tokens
    
    const promptCost = (tokenUsage.prompt_tokens * PROMPT_TOKEN_COST).toFixed(6);
    const completionCost = (tokenUsage.completion_tokens * COMPLETION_TOKEN_COST).toFixed(6);
    const totalCost = (Number(promptCost) + Number(completionCost)).toFixed(6);
    
    return `$${totalCost} (Prompt: $${promptCost}, Completion: $${completionCost})`;
  };
    ...
    <div className={styles.container} role="main">
      {showAuthMessage ? (
        ...
        <Stack horizontal className={styles.chatRoot}>
          ...
          {/* Learning Panel */}
          <Stack.Item className={styles.learningPanel}>
            <LearningPanel />
          </Stack.Item>
        </Stack>
      )}

      {showApiVisualizer && apiVisualizerData && (
        <ApiVisualizer
          request={apiVisualizerData.request}
          response={apiVisualizerData.response}
          onClose={() => setShowApiVisualizer(false)}
        />
      )}
    </div>
  )
